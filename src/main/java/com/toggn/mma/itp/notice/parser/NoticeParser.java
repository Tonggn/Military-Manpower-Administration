...
public class NoticeParser {
    ...
    private static NoticeParseResponse parseNotice(final Element item) {
        ...
        final String businessCode = item.select("eopjongGbcd").text();
        final String welfare = item.select("bokrihs").text();
        final String salaryCode = item.select("gyjogeonCd").text();
        ...
        final String agentCode = item.select("yowonGbcd").text();
        ...
        return new NoticeParseResponse(title, task, businessCode, welfare, salaryCode, serviceAddress,
                highestEducationLevel, experienceDivision, serviceStatusCode, agentCode, enterpriseName, noticeNumber,
